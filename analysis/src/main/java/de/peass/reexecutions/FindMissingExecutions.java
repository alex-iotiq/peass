...
public class FindMissingExecutions {
   ...
   static void findMissing(final String project, File reexecuteFolder, RepoFolders folders) throws IOException, JsonParseException, JsonMappingException, JAXBException {
      ...
      final File folder = new File(folders.getCleanDataFolder(), project);
      if (folder.exists()) {
         int countAll = allExecutions(tests);

         System.out.println(folder.getAbsolutePath());

         removeFinishedExecutions(tests, folder);
         ...
         DivideVersions.generateExecuteCommands(dependencies, tests, NAME, outputStream);
         outputStream.flush();
         outputStream.close();
      }
   }

   public static void removeFinishedExecutions(final ExecutionData tests, final File folder) throws JAXBException {
